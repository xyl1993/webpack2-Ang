export default ['$scope', '$state', '$http', 'APPBASE', '$compile', 'workFlowServ',
    function ($scope, $state, $http, APPBASE, $compile, workFlowServ) {
        ...
        $scope.ccFormData = {};    //form表单model
        $scope.approvalList = [];     //选择的审批人集合
        
        (function () {
            var startDate,endDate;
            $.datetimepicker.setLocale('zh'); 
            $('#ccStartTime').datetimepicker({
                format:'Y-m-d H:i',
                minDate:0,
                onShow:function( ct ){
                     this.setOptions({
                        maxDate:$('#ccEndTime').val()?$('#ccEndTime').val():false
                    })
                }
            });
            $('#ccEndTime').datetimepicker({
                format:'Y-m-d H:i',
                onShow:function( ct ){
                    this.setOptions({
                        minDate:$('#ccStartTime').val()?$('#ccStartTime').val():0
                    })
                }
            });
        ...
        $scope.uploadFile = function(event){
            ...
            var fileInput = $('#ccUploadFileInput');
        ...
        $scope.submit = function(){
            ...
            workFlowServ.saveFormCc($http,APPBASE,formData).then(function(res){
                if(res.data.code === 0){
