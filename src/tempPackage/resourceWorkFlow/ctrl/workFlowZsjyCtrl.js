export default ['$scope', '$state','$http', 'APPBASE', '$compile', 'workFlowServ',
    function ($scope, $state, $http, APPBASE, $compile, workFlowServ) {
        ...
        $scope.zsjyFormData = {};    //form表单model
        $scope.approvalList = [];     //选择的审批人集合
        
        (function () {
            $.datetimepicker.setLocale('zh'); 
            $('#zsjyDate').datetimepicker({
                format:'Y-m-d H:i',
                minDate:0
            });
            ...
            //获取证书类型
            workFlowServ.selectDictionaryInfo($http, APPBASE, '1004').then(function (res) {
                if(res.data.code === 0){
                    $scope.selZsjyObj.typeList = res.data.data;
                }
            })
        ...
        $scope.uploadFile = function(event){
            ...
            var fileInput = $('#zsjyUploadFileInput');
        ...
        $scope.typeSureClick = function(type,data){
            $scope.selZsjyObj.selName = data.name;
            $scope.zsjyFormData.type = data.id;
        };
        /**
         * 确定选择审批人
         */
        ...
        $scope.submit = function(){
            let formData = $scope.zsjyFormData;
            ...
            workFlowServ.saveFormZsjy($http,APPBASE,formData).then(function(res){
                if(res.data.code === 0){
        ...
        $scope.dialogSureClick = function(){
            $state.reload('main.workFlow.zsjy');
