export default ['$scope', '$state','$http', 'APPBASE', '$compile', 'workFlowServ',
    function ($scope, $state, $http, APPBASE, $compile, workFlowServ) {
        ...
        $scope.yyFormData = {};    //form表单model
        $scope.approvalList = [];     //选择的审批人集合
        
        (function () {
            $.datetimepicker.setLocale('zh'); 
            $('#yySealTime').datetimepicker({
                format:'Y-m-d H:i',
                minDate:0
            });
            ...
            //获取用印类型
            workFlowServ.selectDictionaryInfo($http, APPBASE, '1003').then(function (res) {
                if(res.data.code === 0){
                    $scope.selYyObj.typeList = res.data.data;
                }
            })
       ...
       $scope.fileObj = {
        ...
        $scope.uploadFile = function(event){
            ...
            var fileInput = $('#yyUploadFileInput');
        ...
        $scope.typeSureClick = function(type,data){
            $scope.selYyObj.selName = data.name;
            $scope.yyFormData.type = data.id;
        };
        /**
         * 确定选择审批人
         */
        ...
        $scope.submit = function(){
            ...
            workFlowServ.saveFormYy($http,APPBASE,formData).then(function(res){
                if(res.data.code === 0){
